...
import { tools, reels, Technology, ReelModel, Brand } from '@/lib/reel-data';
import { Info, Check, Filter, ShoppingBag, ArrowRight, X } from 'lucide-react';
...
export default function ReelExpertPage() {
    ...
    const filteredReels = reels.filter(reel => {
        ...
        // 2. Tech Match (Must have ALL selected techs)
    ...
    return (
        <main className="min-h-screen bg-[#0f172a] text-slate-300 font-sans selection:bg-cyan-500/30 pb-20">
            ...
            <div className="bg-slate-900 border-b border-slate-800 pt-24 pb-12">
                <div className="container mx-auto px-4 text-center">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-900/30 border border-cyan-800 text-cyan-400 text-xs font-medium mb-6">
                        <Filter className="w-3 h-3" /> Makine Laboratuvarı v1.0
                    </div>
                    <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
                        Teknoloji Uzmanı
                    </h1>
                    <p className="text-lg text-slate-400 max-w-2xl mx-auto">
                        Makinelerin içindeki teknolojileri keşfedin. Ne işe yaradıklarını öğrenin ve sadece ihtiyacınız olan özelliklere sahip makiye ulaşın.
                    </p>
                </div>
            </div>
            ...
            <div className="container mx-auto px-4 py-8 flex flex-col lg:flex-row gap-8">
                ...
                {/* LEFT COLUMN: TECH SELECTOR */}
                <div className="lg:w-1/3 space-y-8">

                    {/* Brand Filter */}
                    <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
                        <h3 className="text-white font-semibold mb-4 flex items-center gap-2">
                            <Filter className="w-4 h-4 text-cyan-500" /> Marka Filtrele
                        </h3>
                        <div className="flex gap-2">
                            {(['All', 'Shimano', 'Daiwa', 'Okuma'] as const).map(brand => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${selectedBrand === brand
                    ...
                    <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-white font-semibold">Teknolojiler</h3>
                    ...
                    `}
                                    ...
                                    >
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <span className={`text-xs font-bold uppercase tracking-wider mb-1 block ${tech.brand === 'Shimano' ? 'text-blue-400' :
                                        ...
                                        <p className="text-xs text-slate-500 mt-2 line-clamp-2">{tech.summary}</p>
                                    </div>
                                ))}
                        </div>
                    </div>
                ...
                <div className="lg:w-2/3">
                    <div className="mb-6 flex items-center justify-between">
                        <h2 className="text-2xl font-bold text-white">
                            Eşleşen Makineler <span className="text-slate-500 text-lg font-normal">({filteredReels.length})</span>
                        </h2>
                    </div>

                    {filteredReels.length === 0 ? (
                        <div className="text-center py-20 bg-slate-900/30 rounded-3xl border border-slate-800 border-dashed">
                            <p className="text-slate-500">Seçili kriterlere uygun makine bulunamadı.</p>
                            <button onClick={() => setSelectedTechs([])} className="mt-4 text-cyan-400 hover:underline">Filtreleri Temizle</button>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {filteredReels.map(reel => (
                                <div key={reel.id} className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden hover:border-slate-600 transition-all group">
                                    {/* Image Area Placeholder */}
                                    <div className="h-48 bg-slate-950 flex items-center justify-center relative">
                                        ...
                                        <div className="absolute top-4 right-4 bg-slate-900/80 backdrop-blur px-3 py-1 rounded-full text-xs font-medium text-white border border-slate-700">
                                            {reel.priceRange === 'premium' ? '👑 Premium' : reel.priceRange === 'value' ? '⚡ F/P Kralı' : '💰 Bütçe Dostu'}
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-6">
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <span className="text-sm font-bold text-emerald-400">{reel.price}</span>
                                        </div>

                                        <p className="text-slate-400 text-sm mb-6 h-10 line-clamp-2">{reel.description}</p>
                                        ...
                                        <a
                                            ...
                                            className="flex items-center justify-center gap-2 w-full py-3 bg-gradient-to-r from-blue-600 to-cyan-600 rounded-xl text-white font-bold hover:from-blue-500 hover:to-cyan-500 transition-all shadow-lg shadow-cyan-900/20"
                                        >
                                            <ShoppingBag className="w-4 h-4" /> Amazon&apos;da İncele
                                        </a>
                                    </div>
                                </div>
                            ))}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setActiveTech(null)}>
                    <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-lg w-full p-8 relative shadow-2xl animate-in fade-in zoom-in duration-300" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="mb-6">
                            ...
                            <p className="text-lg text-slate-400">{activeTech.summary}</p>
                        </div>

                        <div className="space-y-6">
                            <div className="bg-slate-950/50 p-5 rounded-xl border border-slate-800">
                                <h4 className="text-sm font-bold text-white mb-2 flex items-center gap-2">
                                    <Info className="w-4 h-4 text-cyan-500" /> Uzman Açıklaması
                                </h4>
                            ...
                            <div className="bg-emerald-900/20 p-5 rounded-xl border border-emerald-500/20">
                                <h4 className="text-sm font-bold text-emerald-400 mb-2 flex items-center gap-2">
                                    <Check className="w-4 h-4" /> Size Faydası
                                </h4>
                        ...
                        <div className="mt-8 pt-6 border-t border-slate-800 text-center">
                            ...
                            >
                                {selectedTechs.includes(activeTech.id) ? 'Bu Özelliği Filtreden Çıkar' : 'Bu Özelliği Olan Makineleri Bul'}
                            </button>
                        </div>
                    </div>
                </div>
